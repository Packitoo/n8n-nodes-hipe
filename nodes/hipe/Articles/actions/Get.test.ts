...
describe('Articles Get action', () => {
	it('should call helpers.requestWithAuthentication and return correct data (happy path)', async () => {
		const mockThis = {
			getCredentials: async () => ({ url: 'https://fake.api' }),
			helpers: {
				requestWithAuthentication: {
					call: jest.fn().mockResolvedValue({ id: '1', name: 'Test Article' }),
				},
			},
			getNodeParameter: (name: string) => (name === 'articleId' ? '1' : undefined),
		...
		expect(result[0].json).toEqual({ id: '1', name: 'Test Article' });
	});
