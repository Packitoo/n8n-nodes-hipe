...
describe('CorrugatedMaterialCompositionPrices List uses shared pagination', () => {
	...
	it('returnAll true: passes undefined limit and correct endpoint', async () => {
		const self = makeThis({ returnAll: true, filters: { currency: 'EUR' }, sort: { sortBy: 'validFrom', sortOrder: 'asc' }, page: 1 });
		const res = await (execute as any).call(self, [{ json: {} }]);
		expect(pagination.listWithPagination).toHaveBeenCalledWith(self, '/api/corrugated-material-composition-prices', {
			returnAll: true,
			limit: undefined,
			page: 1,
