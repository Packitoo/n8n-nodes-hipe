...
describe('CorrugatedMaterialCompositions Create credential error handling', () => {
  function makeThis(continueOnFail: boolean, creds: any, helpers?: any): IExecuteFunctions {
    return {
      getCredentials: async () => creds,
      getNodeParameter: (name: string) => {
        if (name === 'corrugatedMaterial') return 'material_1';
        if (name === 'flute') return 'flute_1';
        if (name === 'liners') return ['liner_1', 'liner_2'];
