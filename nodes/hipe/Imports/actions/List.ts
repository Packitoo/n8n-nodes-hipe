...
export const properties: INodeProperties[] = [
	...
	},
	{
		...
		default: {},
		description:
			'Flat filter keys per OpenAPI (e.g. search, status, type, delimiter, filterId, date windows). "Search" maps to "s" query parameter.',
		displayOptions: {
			show: {
				resource: ['import'],
				operation: ['getMany'],
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		try {
			...
			const response = await listWithPaginationFlat(this, '/api/imports', {
				returnAll,
				limit,
				page: uiPage,
				filters,
