...
describe('Exports Archive action', () => {
	it('should call helpers.requestWithAuthentication and return correct data (happy path)', async () => {
		...
		expect(mockThis.helpers.requestWithAuthentication.call).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'PATCH',
				url: 'https://fake.api/api/exports/export-123/archive',
				json: true,
			}),
		);
		expect(result[0].json).toEqual({ id: 'export-123', archived: true });
	});
	...
	it('should properly encode special characters in ID', async () => {
		const mockThis = {
			...
			getNodeParameter: (name: string, i: number) => {
				if (name === 'id') return 'export/with/slashes';
				return undefined;
			},
			continueOnFail: () => false,
		} as any;
		...
		expect(mockThis.helpers.requestWithAuthentication.call).toHaveBeenCalledWith(
			...
			'hipeApi',
			expect.objectContaining({
				url: 'https://fake.api/api/exports/export%2Fwith%2Fslashes/archive',
			}),
		);
	});
