...
describe('Projects List action', () => {
	...
	it('should pass page/limit/filters and sort to flat qs (non-returnAll)', async () => {
		const mockThis = {
			...
			helpers: {
				requestWithAuthentication: {
					call: jest.fn().mockResolvedValue({ data: [{ id: '3', name: 'Gamma' }] }),
				},
			},
			getNodeParameter: (name: string) => {
				...
				if (name === 'filters') return { name: 'ga', clientId: 'c1' };
		...
		expect(mockThis.helpers.requestWithAuthentication.call).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'GET',
				url: 'https://fake.api/api/projects/pagination',
	...
	it('should iterate all pages when returnAll is true using pageCount', async () => {
		const page1 = Array.from({ length: 100 }, (_, i) => ({ id: `p${i + 1}` }));
		const page2 = [{ id: 'p101' }];
		const mockThis = {
			getCredentials: async () => ({ url: 'https://fake.api' }),
			helpers: {
