...
describe('Imports GetTypes action', () => {
	it('should call helpers.requestWithAuthentication and return types data (happy path)', async () => {
		const mockTypesResponse = [
			{ id: 1, name: 'CSV Import', description: 'Import CSV files' },
			{ id: 2, name: 'Excel Import', description: 'Import Excel files' },
		];
		...
		expect(mockThis.helpers.requestWithAuthentication.call).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'GET',
				url: 'https://fake.api/api/imports/utils/get-types',
				json: true,
			}),
		);
